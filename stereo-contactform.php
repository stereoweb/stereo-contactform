...
if (!class_exists('ST_ContactForm')) {
    ...
    class ST_ContactForm
    {
        ...
        public function post_contact()
        {
            ...
            add_post_meta($id, 'form_data', apply_filters('st_cf_post_info', $forminfo, $id));
        ...
        public function send_email($forminfo, $subject, $postid)
        {
            ...
            if (isset($_FILES['_file']) && count($_FILES['_file']['name'])) {
                for ($x = 0; $x < count($_FILES['_file']['name']); $x++) {
                    if (is_uploaded_file($_FILES['_file']['tmp_name'][$x])) {
                        $file = sys_get_temp_dir() . '/' . sanitize_file_name($_FILES['_file']['name'][$x]);
                        if (@move_uploaded_file($_FILES['_file']['tmp_name'][$x], $file)) {
                            $files[] = [
                                'file' => $file,
                                'no_upload' => true,
                            ];
            ...
            wp_mail($to, $subject, $html, $headers, $sendFiles);
        ...
        public function save_files($files,$postid)
        {
            ...
            $existingFiles = get_field('uploadedfiles',$postid);
            if (is_array($existingFiles)) {
                $uploadedfiles = array_merge($uploadedfiles,$existingFiles);
            }
        ...
        {
            add_meta_box('metabox_files', 'Fichier(s) transféré(s)', [$this, 'metabox_files_content'], 'st_contactform', 'side', 'high');
        }
